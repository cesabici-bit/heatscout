...
class ThermalStream:
    ...
    def _validate(self):
        ...
        if not self.fluid_type or not self.fluid_type.strip():
            raise ValueError("Fluid type must not be empty")

        # Temperatura minima: zero assoluto
        if self.T_in <= -273.15:
            raise ValueError(f"T_in={self.T_in}°C is below absolute zero (-273.15°C)")
        if self.T_out <= -273.15:
            raise ValueError(f"T_out={self.T_out}°C is below absolute zero (-273.15°C)")

        if self.T_in == self.T_out:
            raise ValueError(f"T_in and T_out are equal ({self.T_in}°C): no heat exchange occurs")

        # Per hot_waste: T_in > T_out (il fluido si raffredda)
        if self.stream_type == StreamType.HOT_WASTE and self.T_in < self.T_out:
            raise ValueError(
                f"f"Stream 'hot_waste': T_in ({self.T_in}°C) must be greater than T_out ({self.T_out}°C)"
            )

        # Per cold_demand: T_out > T_in (il fluido si scalda)
        if self.stream_type == StreamType.COLD_DEMAND and self.T_out < self.T_in:
            raise ValueError(
                f"Stream 'cold_demand': T_out ({self.T_out}°C) must be greater than T_in ({self.T_in}°C)"
            )

        if self.mass_flow <= 0:
            raise ValueError(f"mass_flow={self.mass_flow} kg/s must be greater than 0")

        if not (0 < self.hours_per_day <= 24):
            raise ValueError(f"hours_per_day={self.hours_per_day} must be between 0 and 24")

        if not (0 < self.days_per_year <= 366):
            raise ValueError(f"days_per_year={self.days_per_year} must be between 0 and 366")

        if self.pressure <= 0:
            raise ValueError(f"pressure={self.pressure} Pa must be greater than 0")
