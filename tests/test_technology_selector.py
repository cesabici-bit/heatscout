...
class TestTechnologySelection:
    def test_fumi_400_suggests_multiple(self):
        ...
        s = ThermalStream(
            "Fumi", "fumi_gas_naturale", 400, 180, 2.0, 16, 250, StreamType.HOT_WASTE
        )
    ...
    def test_acqua_55_suggests_heat_pump(self):
        ...
        s = ThermalStream(
            "Acqua", "acqua", 55, 30, 2.0, 16, 250, StreamType.HOT_WASTE
        )
        recs = select_technologies(s)
        tech_ids = [r.technology.id for r in recs]
    ...
    def test_vapore_120_suggests_scambiatore(self):
        ...
        s = ThermalStream(
            "Vapore flash", "fumi_gas_naturale", 120, 80, 0.5, 12, 300, StreamType.HOT_WASTE
        )
        recs = select_technologies(s)
        tech_ids = [r.technology.id for r in recs]
    ...
    def test_recommendations_sorted_by_savings(self):
        ...
        s = ThermalStream(
            "Fumi", "fumi_gas_naturale", 400, 150, 2.0, 16, 250, StreamType.HOT_WASTE
        )
        recs = select_technologies(s)
        if len(recs) > 1:
    ...
    def test_all_recommendations_have_positive_savings(self):
        ...
        s = ThermalStream(
            "Acqua", "acqua", 80, 40, 1.0, 16, 250, StreamType.HOT_WASTE
        )
        recs = select_technologies(s)
        for r in recs:
